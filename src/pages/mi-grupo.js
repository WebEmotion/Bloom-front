    ...
    observer(({ RootStore }) => {
        ...
        const reload = async () => {
            ...
            if (response.success) {
                ...
                setOriginalTitle(response.data[1].toUpperCase())
                setTitle(response.data[1].toUpperCase())
            }
        }
        ...
        const inviteMember = async () => {
            ...
            const response = await MeAPI.inviteGroupMember(store.token, email)
        ...
        const removeMember = async () => {
            ...
            const response = await MeAPI.removeGroupMember(store.token, deleteId)
            if (response.success) {
                await reload()
            } else {
                toast && toast.current && toast.current.show({ severity: 'error', summary: 'Lo sentimos', detail: response.message });
        ...
        useEffect(() => {
            const load = async () => {
                ...
                const me = await MeAPI.me(store.token)
                if (me.success) {
                    setMe(me.data.profile)
                }
                setLoading({
                    animateIn: false,
                    loading: false
        ...
        const updateName = async () => {
            ...
            const rename = await MeAPI.changeGroupName(store.token, title)
            if (rename.success) {
                setEdited(false)
                await reload()
            }
                ...
                <Layout page="perfil">
                    ...
                    <Dialog header="Añadir nuevo miembro al grupo" className="spDialog" visible={addMember} onHide={() => { setAddMember(false); setEmail('') }}>
                        <div className="p-d-flex p-flex-column p-jc-center p-ai-center">
                            <p><strong>Al añadir un nuevo miembro al grupo, le darás acceso a todas tus reservaciones grupales disponibles. No añadas a extraños y asegúrate de introducir el correo electrónico correcto.</strong></p>
                    ...
                    <Dialog header="Eliminar miembro" className="spDialog" visible={showRemoveMember} onHide={() => { setShowRemoveMember(false); setDeleteId(null) }}>
                        <div className="p-d-flex p-flex-column p-jc-center p-ai-center">
                            <p>¿Estás seguro de eliminar a este miembro de tu grupo? Perderá acceso a todas las reservaciones grupales disponibles. Solo puedes hacer cambios en tu grupo una vez.</p>
                        ...
                        <div className="p-d-flex p-flex-column p-jc-center p-ai-center">
                            ...
                            <div className="p-d-flex p-jc-evenly">
                                {navigator.share && <img src="https://img.icons8.com/fluent/200/000000/share.png" style={{
                                    ...
                                    margin: 10,
                                    cursor: 'pointer'
                                }} title="Copiar al portapapeles" onClick={() => {
                                    if (navigator.share) {
                                        navigator.share({
                    ...
                    <div className="p-grid p-align-center">
                        <div className="p-col-12" style={{ position: 'relative' }}>
                            <Button
                                ...
                                className="p-button"
                                disabled={!edited}
                                style={{ width: "auto", position: 'fixed', right: 30, top: 20, color: 'white', borderRadius: 10, backgroundColor: '#788ba5', border: 'none', transition: '300ms linear all', transform: edited ? "translateY(0px)" : "translateY(-100px)" }}
                                onClick={() => {
                                    updateName()
                                }}
                            />
                            <p style={{ textAlign: 'center', marginBottom: 0, fontWeight: 'bold' }}>MI GRUPO</p>
                            <InputText value={title} placeholder="Nombre de tu grupo" style={{
                                marginLeft: "25%",
                                fontSize: 30,
                                fontWeight: 'bold',
                    ...
                    <div className="p-grid">
                        ...
                        <div className="p-col-12">
                            <p style={{color: '#ccc'}}>Nota: Puedes hacer cambios a los miembros de tu grupo <b>solo una vez</b>. Si deseas hacer más cambios, dirigete al front-desk.</p>
                        </div>
                    </div>
