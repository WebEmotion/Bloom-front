    ...
    return (
      <div>
        ...
        >
          ...
          <p className="classes-taken" style={{ color: forbidden && isReserved ? '#495057' : isReserved && !disabled ? '#495057' : forbidden ? '#FFFFFF' : (disabled || now >= end) ? '#00000030' : '#495057', fontWeight: 'bold', opacity: forbidden && isReserved ? 1.0 : isReserved && !disabled ? 1.0 : forbidden ? 0.5 : !havePackage && !canReservate ? 0.5 : 1.0 }}>{item.occupied}/16</p>
          <p style={{ color: forbidden && isReserved ? '#ffffff' : isReserved && !disabled ? '#ffffff' : forbidden ? '#FFFFFF' : (disabled || now >= end) ? '#00000050' : !havePackage && !canReservate ? '#000000' : '#000000', margin: 0, opacity: forbidden && isReserved ? 1.0 : isReserved && !disabled ? 1.0 : forbidden ? 0.5 : !havePackage && !canReservate ? 0.5 : 1.0, fontSize: 12 }}>{hour}</p>
          <p style={{ color: forbidden && isReserved ? '#495057' : isReserved && !disabled ? '#495057' : forbidden ? '#FFFFFF' : (disabled || now >= end) ? '#00000030' : !havePackage && !canReservate ? '#000000' : '#000000', margin: 0, fontWeight: 'bold', opacity: forbidden && isReserved ? 1.0 : isReserved && !disabled ? 1.0 : forbidden ? 0.5 : !havePackage && !canReservate ? 0.5 : 1.0, fontSize: 12 }}>{instructor}</p>
          ...
          <p style={{ color: forbidden && isReserved ? '#ffffff' : isReserved && !disabled ? '#ffffff' : forbidden ? '#FFFFFF' : (disabled || now >= end) ? '#00000030' : !havePackage && !canReservate ? '#000000' : '#000000', margin: 0, opacity: forbidden && isReserved ? 1.0 : isReserved && !disabled ? 1.0 : forbidden ? 0.5 : !havePackage && !canReservate ? 0.5 : 1.0, fontSize: 12 }}>{item.Rooms.name}</p>
        </div>
      </div>
    )
}
  ...
  observer(({ RootStore }) => {
    ...
    return (
        ...
        {showCalendar && <div className="indicators" style={{
            ...
            <Indicator style={{ marginLeft: 10 }} color="#E6CDB5" label="Clases fuera de vigencia/Sin clases" />
        </div>}
