...
import MisClases from "../assets/images/rode-1.png"
  ...
  return (
    <div>
      <div 
        ...
        style={{ 
          backgroundColor: forbidden && isReserved ? '#6e6666' : forbidden ? '#3eb978' : disabled ? '#00000020' : isReserved ? '#6e6666' : now >= end ? '#00000020' : !havePackage && !canReservate ? '#3eb978' : '#3eb978', 
        ...
        <p style={{ color: forbidden && isReserved ? '#ffffff' : isReserved && !disabled ? '#ffffff' : forbidden ? '#FFFFFF' : (disabled || now >= end) ? '#00000050' : !havePackage && !canReservate ? '#FFFFFF' : '#000000', margin: 0, opacity: forbidden && isReserved ? 1.0 : isReserved && !disabled ? 1.0 : forbidden ? 0.5 : !havePackage && !canReservate ? 0.5 : 1.0, fontSize: 12 }}>{hour}</p>
        <p style={{ color: forbidden && isReserved ? '#495057' : isReserved && !disabled ? '#495057' : forbidden ? '#FFFFFF' : (disabled || now >= end) ? '#00000030' : !havePackage && !canReservate ? '#FFFFFF' : '#000000', margin: 0, fontWeight: 'bold', opacity: forbidden && isReserved ? 1.0 : isReserved && !disabled ? 1.0 : forbidden ? 0.5 : !havePackage && !canReservate ? 0.5 : 1.0, fontSize: 12 }}>{instructor}</p>
        ...
        <p style={{ color: forbidden && isReserved ? '#ffffff' : isReserved && !disabled ? '#ffffff' : forbidden ? '#FFFFFF' : (disabled || now >= end) ? '#00000030' : !havePackage && !canReservate ? '#FFFFFF' : '#000000', margin: 0, opacity: forbidden && isReserved ? 1.0 : isReserved && !disabled ? 1.0 : forbidden ? 0.5 : !havePackage && !canReservate ? 0.5 : 1.0, fontSize: 12 }}>{item.Rooms.name}</p>
      </div>
    </div>
  )
}
    ...
    const loadClassesItems = () => {
      ...
      if (myClasses) {
        for (var i in myClasses.bookings) {
          ...
          if (location === 'Indoors'  || location === 'Outdoors') continue 
          for (var d in dates) {
            const _date = dates[d]
